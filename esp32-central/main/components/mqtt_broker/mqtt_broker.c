    ...
    switch ((esp_mqtt_event_id_t) event_id) {
        ...
        case MQTT_EVENT_DATA: {
            ...
            if (json == NULL) {
                ...
                if (cJSON_IsString(status_item) && cJSON_IsString(port_item) && cJSON_IsString(type_item)) {
                    ...
                    if (strcmp(type_str, "LED") == 0) {
                        int gpio_num = -1;
                        if (strncmp(port_str, "GPIO", 4) == 0) {
                            gpio_num = atoi(port_str + 4);
                        }

                        if (gpio_num >= 0) {
                            if (strcmp(status_str, "on") == 0) {
                                ESP_LOGI(TAG, "Status urządzenia: ON");
                                device_control_set_led(gpio_num, true);
                            } else if (strcmp(status_str, "off") == 0) {
                                ESP_LOGI(TAG, "Status urządzenia: OFF");
                                device_control_set_led(gpio_num, false);
                            } else {
                                ESP_LOGW(TAG, "Nieznany status: %s", status_str);
                            }
                        } else {
                            ESP_LOGW(TAG, "Niepoprawny port: %s", port_str);
                        }
                    } else {
                        ESP_LOGI(TAG, "Nieobsługiwany typ: %s", type_str);
                    }
                } else {
                    ESP_LOGW(TAG, "Brak wymaganych pól w otrzymanym JSON (status/port/type).");
                }
